...
public class VoxelData
{
    ...
    public VoxelData(int size, Vector3 originPosition, Vector3 offset)
    {
        if(size > Int32.MaxValue)
        {
            Debug.LogError("Chunk size too large");
        }
        else
        {
            _size = size;
    ...
    void Initialize(float[,,] array, Vector3 origin, Func<Vector3, float> initializer, Vector3 offset)
    {
        //Vector3 position;
    ...
    {
        return Pos.x * Pos.x + Pos.y * Pos.y + Pos.z * Pos.z - Mathf.Sqrt(_size);
    }
    ...
    {
        float val = (PerlinNoise3D(v.x, v.y, v.z) * .5f) + .5f;
        Debug.Log($"Value {val} for vector {v}");
        return val >= 0f ? 1 : 0;
    }
